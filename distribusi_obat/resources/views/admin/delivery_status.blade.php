...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold">Kelola Status Pengiriman</h4>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-0">
                        ...
                        <input type="text" id="status_name" class="form-control border-0 bg-light"
                               placeholder="Contoh: On Delivery" required>
                    </div>
                </div>
...
<script>
    ...
    function fetchStatuses() {
        showSkeletons();
        axios.get('/api/admin/delivery-statuses')
            .then(res => {
                let html = '';
                res.data.forEach((s, i) => {
                    ...
                    <tr class="border-bottom">
                        ...
                        <td class="text-center">
                            <button onclick="editStatus(${s.id}, '${s.name}')"
                ...
                document.getElementById('statusTableBody').innerHTML =
                    html || '<tr><td colspan="3" class="text-center py-4 text-muted">Belum ada status.</td></tr>';
            })
            .catch(() => {
                document.getElementById('statusTableBody').innerHTML =
                    '<tr><td colspan="3" class="text-center py-4 text-danger">Gagal memuat data.</td></tr>';
            });
    }
    ...
    function saveStatus(e) {
        ...
        const data = { name: document.getElementById('status_name').value };
        const req  = id
            ? axios.put(`/api/admin/delivery-statuses/${id}`, data)
            : axios.post('/api/admin/delivery-statuses', data);
    ...
    function deleteStatus(id) {
        Swal.fire({ title: 'Hapus Status?', icon: 'warning', showCancelButton: true })
            .then(res => {
                if (res.isConfirmed) {
                    axios.delete(`/api/admin/delivery-statuses/${id}`)
                        .then(() => fetchStatuses())
